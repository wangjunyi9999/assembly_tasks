...
class Simulation:
    def __init__(self, model="model/ur5gripper.xml", nsubsteps=2):
        """
        ...
        self.arm_joint_names=[name for name in self.model.joint_names if "UR5" in name and "base" not in name] # 6
        self.gripper_joint_names=[name for name in self.model.joint_names if "base" in name] # 2
        ...
        """
        dict_body contents:
        {'UR5:base_link': 3, 'UR5:shoulder_link': 4, 'UR5:upper_arm_link': 5, 'UR5:forearm_link': 6, 
        'UR5:wrist_1_link': 7, 'UR5:wrist_2_link': 8, 'UR5:wrist_3_link': 9, 'UR5:ee_link': 10, 'UR5:robotiq_85_base_link': 11, 
        'UR5:left_inner_knuckle': 12, 'UR5:left_inner_finger': 13, 'UR5:right_inner_knuckle': 14, 'UR5:right_inner_finger': 15, 'object': 16}
        """
        ...
        self.t=0
        self.sim.model.opt.timestep=0.0001#0.0001#0.002
        self.T=100
        self.n_steps=0
        self.catch_t=10#10
        self.raise_t=10#10
    ...
    def move(self, m_action):
        ...
        self.sim.data.qpos[self.n_arms:self.n_all]=1#self.sim.data.qvel[self.n_arms:self.n_all]*(1+self.n_steps)
    ...
    def grasp(self, m_action, g_action):
        ...
        if self.n_steps <= self.T - self.catch_t- self.raise_t: # down the gripper
            ...
            self.sim.data.qpos[self.n_arms:self.n_all]=0#-temp/grasp_t*(self.n_steps-self.moving_frame)
            ...
            self.sim.data.qvel[6:8]=-0.01
            self.sim.data.qpos[6:8]=-0.01*catch_t
            #print(f'actuator for two fingers: {self.sim.data.ctrl[-2:]}')
            ...
            self.sim.data.qvel[self.n_arms:self.n_all]=0
            self.sim.data.qpos[self.n_arms:self.n_all]=-0.5
